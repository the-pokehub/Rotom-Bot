...
from translator.models import TranslatedPart
...
def format_response_legacy(a):
    ...
    li_data = json.loads(fi_data.split('translator')[1].replace('"[', '[').replace(']"', ']'))
    return li_data
...
def _format_response(a):
    ...
    li_data = json.loads(fi_data.split('translator')[1], strict=False)
    return li_data
