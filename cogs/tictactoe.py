        ...
        while move < 1 or move > 9:
            try:
                payload = await self.bot.wait_for(
                    "raw_reaction_add",
                    check=lambda x: x.message_id == self.message.id and x.user_id == self.turn.id and x.emoji.name in self.emojis,
                    timeout=60,
                )
        ...
        if self.is_enemy_human:
            ...
            try:
                msg = await self.bot.wait_for(
                    "message",
                    check=lambda x: (
                        x.author == self.ctx.author and x.content.lower().startswith(("y", "n", "q", "r"))
                    ),
                    timeout=60,
                )
...
class TTT(commands.Cog):
    ...
    async def loading(self, ctx, time: int):
        ...
        try:
            for i in range(1, len(loadingLst)):
    ...
    @commands.hybrid_command(name="tictactoe", aliases=["tic-tac-toe", "tic_tac_toe", "ttt"])
...
async def setup(client):
    await client.add_cog(TTT(client))
